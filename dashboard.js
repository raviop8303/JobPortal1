...
function applyForJob(jobId) {
    ...
    if (!isAuthenticated || userRole !== 'jobseeker') {
        if (typeof showToast === 'function') showToast('Please login as a job seeker to apply.', {type:'warn'});
        else alert('Please login as a job seeker to apply.');
        return;
    }

    const job = jobs.find(j => j.id === jobId);
    if (!job) {
        if (typeof showToast === 'function') showToast('Job not found.', {type:'error'});
    ...
    if (job.applications.includes(username)) {
        if (typeof showToast === 'function') showToast('You have already applied for this job.', {type:'info'});
    ...
    if (typeof showToast === 'function') showToast(`Successfully applied for ${job.title}.`, {type:'success'});
    else alert(`Successfully applied for ${job.title}.`);
...
function showPostJobForm() {
    ...
    container.innerHTML = `
        <div class="dashboard-view">
            ...
            <form onsubmit="event.preventDefault(); handlePostJob()">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="job-salary" placeholder="e.g., 50k-70k" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <button type="button" class="btn" onclick="loadDashboard()">Cancel</button>
                </div>
            </form>
        </div>
    `;
...
function handlePostJob() {
    const username = localStorage.getItem('userName');
    const profile = profiles.employers[username];
    if (!profile) {
        if (typeof showToast === 'function') showToast('Employer profile not found.', {type:'error'});
        return;
    }
    ...
    const newJob = {
        ...
        applications: [],
        newlyAdded: true // Highlight new job
    };

    // Add to global jobs list
    jobs.unshift(newJob);
    // Add to employer's posted jobs
    profile.postedJobs.push(newJob.id);

    if (typeof showToast === 'function') showToast('Job posted successfully!', {type:'success'});

    // Refresh main job listings and dashboard
...
function viewApplicants(jobId) {
    ...
    if (job.applications.length > 0) {
        applicantsHtml = `
            <ul class="dashboard-list">
                ${job.applications.map(username => {
                    const seekerProfile = profiles.jobseekers[username] || { name: username, skills: [] };
                    return `
                        <li>
                            <strong>${seekerProfile.name}</strong> (${username})
                            <p>Skills: ${seekerProfile.skills.join(', ')}</p>
                            <button class="btn-secondary">View Resume</button>
                        </li>
                    `;
                }).join('')}
            </ul>
    ...
    container.innerHTML = `
        <div class="dashboard-view">
            ...
            <button class="btn" onclick="loadDashboard()">← Back to Dashboard</button>
            <div class="dashboard-section">
                ${applicantsHtml}
            </div>
        </div>
...
function viewCompanyProfile(companyName) {
    ...
    if (!profile) {
        if (typeof showToast === 'function') showToast(`Company profile for "${companyName}" not found.`, {type:'info'});
        return;
    }
    ...
    container.innerHTML = `
        <div class="dashboard-view">
            ...
            <p><strong>Website:</strong> <a href="http://${profile.website}" target="_blank">${profile.website}</a></p>
            
            <div class="dashboard-section">
                <h5>Open Positions at ${profile.name} (${companyJobs.length})</h5>
    ...
    // Open the dashboard panel to show the profile
    const dashboardPanel = document.getElementById('dashboard-section');
    if (dashboardPanel && !dashboardPanel.classList.contains('open')) {
        if (typeof openDashboard === 'function') openDashboard();
        else dashboardPanel.classList.add('open');
    }
}
...
function viewJobDetails(jobId) {
    ...
    container.innerHTML = `
        <div class="dashboard-view">
            <h5>Job Details</h5>
            <button class="btn" onclick="viewCompanyProfile('${job.company}')">← Back to Company</button>
            <div class="dashboard-section">
                <h4>${job.title}</h4>
